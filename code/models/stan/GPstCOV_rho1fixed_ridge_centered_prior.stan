...
parameters {
  ...
  real<lower=0> kappa ; // global shrinkage parameter for ridge prior
}
model{ // prior + likelihood
  ...
  // Ridge Bayesian prior on beta
  kappa ~ gamma(0.5, 0.5); 
  //beta ~ normal(0, 1/kappa);
  beta ~ multi_normal(rep_vector(0, P), (1/kappa) * identity_matrix(P));
